...
window.ZH = {
  ...
  "q.hazard":"地震危险性","q.ss":"典型 Ss","q.s1":"典型 S1","q.topics":"识别的主题","q.site":"场地类别",
  "ctrl.std":"标准体系","ctrl.all":"全部","ctrl.id":"🇮🇩 印尼","ctrl.us":"🇺🇸 美国","ctrl.cn":"🇨🇳 中国",
  "ctrl.proj":"项目数据（可选）","ctrl.risk":"风险类别","ctrl.site":"场地类别","ctrl.auto":"— 自动 —",
  "ctrl.using":"采用您提供的数据：","ctrl.calc":"⚙ 用此参数打开反应谱",
  "sec.applicable":"适用标准",
  ...
  "mod.sources":"来源：",
  "mod.systematic":"📋 下方设计参数按上方设计流程的顺序排列 — 请依次查阅。其他需注意的参数集中在末尾。",
  "mod.additional":"⊕ 其他需注意的设计参数",
  "proc.per":"依据：",
  // AI answer engine
  "ai.title":"工程解答",
...
var TOPIC_ZH={
  ...
  structural_concrete:{name:"结构混凝土",kw:["混凝土","钢筋","保护层","柱","梁","板","剪力墙","配筋","强度等级","砼","强度折减系数","受拉控制","受压控制","净受拉应变","等效应力图","耐久性","暴露等级","硫酸盐","冻融","氯化物","水胶比","抗折模量"]},
  steel_structure:{name:"钢结构",kw:["钢结构","钢材","焊接","螺栓","抗弯框架","支撑","屈曲","稳定","型钢","受压构件","受拉构件","受弯构件","钢柱","钢梁","弯曲屈曲","弯扭屈曲","临界应力","塑性弯矩","截面分类","厚实","宽厚比","剪力滞后","抗力系数","长细比","块状剪切","承压","撕裂","摩擦型","预拉力","角焊缝","坡口焊","组合构件","组合柱","组合梁","栓钉","抗剪连接件","节点板","边距","钢管混凝土","二阶分析","放大系数","稳定支撑","概念荷载","直接分析","疲劳","应力幅","抗火","高温","临界温度"]},
...
var MOD_ZH={
  concrete:{
    ...
    sections:[
      ...
      {h:"强度折减系数 φ（SNI 2847:2019 表 21.2.1）",
       table:{center:true,cols:["作用 / 构件","φ"],
         rows:[
           ["弯矩、轴力或组合","0.65 – 0.90（按表 21.2.2，随 εt）"],
           ["受剪","0.75"],
           ["受扭","0.75"],
           ["承压","0.65"],
           ["后张锚固区","0.85"],
           ...
           ["拉压杆模型（第 23 章）","0.75"],
           ["素混凝土","0.60"],
           ["混凝土锚栓","0.45 – 0.75（按第 17 章）"]
         ]}},
      {h:"φ 随净受拉应变 εt 过渡（SNI 2847:2019 表 21.2.2）",
       table:{center:true,cols:["最外排受拉钢筋 εt","分类","φ（箍筋）","φ（螺旋）"],
         rows:[
           ["εt ≤ εty（fy420 约 0.002）","受压控制","0.65","0.75"],
           ["εty &lt; εt &lt; 0.005","过渡","0.65 + 0.25·(εt−εty)/(0.005−εty)","0.75 + 0.15·(εt−εty)/(0.005−εty)"],
           ["εt ≥ 0.005","受拉控制","0.90","0.90"]
         ]},
       notes:["εty = fy/Es（420 级约 0.0021）。受拉控制截面（εt ≥ 0.005）是受弯设计的延性目标；梁 ρmax 取使 εt ≥ 0.004。"]},
      {h:"等效应力图与设计假定（SNI 2847:2019 §22.2）",
       table:{cols:["项目","取值"],
         rows:[
           ["混凝土极限应变 εcu","0.003"],
           ["等效应力图高度","a = β1·c；受压区 a 范围内混凝土应力 = 0.85·fc'"],
           ["β1 — 17 ≤ fc' ≤ 28 MPa","0.85"],
           ["β1 — 28 &lt; fc' &lt; 55 MPa","0.85 − 0.05·(fc'−28)/7"],
           ["β1 — fc' ≥ 55 MPa","0.65"],
           ["弹性模量 Ec","4700·√fc' MPa（普通混凝土）；0.043·wc^1.5·√fc'"],
           ["抗折模量 fr","0.62·λ·√fc' MPa"],
           ["结构混凝土最小 fc'","17 MPa（§19.2.1.1）"]
         ]}},
      {h:"耐久性 — 暴露等级（SNI 2847:2019 第 19 章，表 19.3.2.1）",
       table:{center:true,cols:["暴露等级","最大水胶比","最小 fc' (MPa)","条件"],
         rows:[
           ["F0 / F1 / F2 / F3（冻融）","— / 0.55 / 0.45 / 0.40","17 / 24 / 31 / 35","冻融、除冰盐"],
           ["S0 / S1 / S2 / S3（硫酸盐）","— / 0.50 / 0.45 / 0.45","17 / 28 / 31 / 31","土/水中水溶性硫酸盐"],
           ["W0 / W1 / W2（水）","— / — / 0.50","17 / 17 / 28","与水接触、低渗透性"],
           ["C0 / C1 / C2（腐蚀）","— / — / 0.40","17 / 17 / 35","钢筋氯化物暴露（含最大 Cl⁻ 限值）"]
         ]},
       notes:["在各类别（F、S、W、C）中取最不利适用等级。F1–F3 须引气；S1–S3 用抗硫酸盐水泥（II/V 型）。保护层也随暴露增大（表 20.6.1.3）。"]},
    ...
    sources:"SNI 2847:2019 表 21.2.1 与 21.2.2（φ）、§22.2 与表 22.2.2.4.3（β1、εcu）、§19.2–19.3（耐久性，表 19.3.2.1）、表 20.6.1.3（保护层）、第 18 章（抗震）— 提取自 PDF · SNI 2052:2017（钢筋） · GB 50010-2010 表 4.1.3、4.1.5 · ACI 318-19（= SNI 2847）"
  },
  steel:{
    title:"钢结构 — 设计参数",
    sections:[
      ...
      {h:"螺栓设计 — 名义应力（SNI 1729:2020 表 J3.2）",
       table:{center:true,cols:["紧固件","Fnt — 抗拉 (MPa)","Fnv — 抗剪 (MPa)"],
         rows:[
           ["A307","310","186"],
           ["A 组（A325）— 螺纹在剪切面内（-N）","620","372"],
           ["A 组（A325）— 螺纹在剪切面外（-X）","620","469"],
           ["B 组（A490）— 螺纹在剪切面内（-N）","780","469"],
           ["B 组（A490）— 螺纹在剪切面外（-X）","780","579"],
           ["C 组（F3043）— 螺纹在剪切面内","1040","620"],
           ["螺纹件（A3.4）— 螺纹在内/外","0.75·Fu","0.450·Fu / 0.563·Fu"]
         ]},
       notes:["螺栓强度 Rn = Fn·Ab，φ = 0.75（Ω = 2.00）。M20 A325-N 单剪：0.75 × 372 × 245 mm² ≈ 68 kN。拉剪组合：F'nt = 1.3·Fnt − (Fnt/φFnv)·frv ≤ Fnt（J3-3a）。"]},
      {h:"螺栓承压、撕裂与摩擦型（SNI 1729:2020 第 J3 章）",
       table:{cols:["极限状态","承载力（φ = 0.75）"],
         rows:[
           ["孔壁承压（考虑变形）","Rn = <strong>2.4·d·t·Fu</strong>（不考虑变形 3.0·d·t·Fu）"],
           ["撕裂（边距/间距）","Rn = <strong>1.2·lc·t·Fu</strong>（不考虑变形 1.5·lc·t·Fu）；lc = 净距"],
           ["最小间距","中心距 ≥ 2⅔·d（宜 3·d）"],
           ["最小边距","按表 J3.4（约 1.25·d 至 1.75·d，随螺栓直径）"],
           ["摩擦型抗滑移","Rn = μ·Du·hf·Tb·ns；<strong>μ = 0.30（A 级面）</strong>/ 0.50（B 级面）；Du = 1.13；φ = 1.00（标准孔）"],
           ["最小预拉力 Tb","表 J3.1（如 M20 A325 = 142 kN，M24 = 205 kN）"]
         ]}},
      {h:"焊缝设计（SNI 1729:2020 第 J2 章）",
       table:{cols:["项目","规定"],
         rows:[
           ["角焊缝强度","Rn = Fnw·Awe，<strong>Fnw = 0.60·FEXX</strong>，φ = 0.75（Ω = 2.00）"],
           ["方向性提高","Fnw = 0.60·FEXX·(1.0 + 0.50·sin¹·⁵θ)，θ = 荷载与焊缝轴夹角"],
           ["有效喉厚（角焊缝）","te = <strong>0.707·（焊脚尺寸）</strong>（等脚手工焊）"],
           ["有效面积 Awe","te × 有效长度；L &gt; 100·w 时长度折减 β = 1.2 − 0.002(L/w) ≤ 1.0"],
           ["完全熔透坡口焊 — 垂直轴向受拉/压","强度 = 母材（匹配焊材）"],
           ["最小角焊缝尺寸","表 J2.4 按较薄件：3 mm（t ≤ 6 mm）→ 8 mm（t &gt; 50 mm）；沿边最大 t（&lt;6mm）或 t−2 mm"]
         ]},
       notes:["E70（FEXX = 485 MPa）角焊缝单位喉厚设计抗剪 = 0.75 × 0.60 × 485 = 218 N/mm²。焊材级别须匹配或高于母材。"]},
      {h:"块状剪切与连接板件强度（SNI 1729:2020 第 J4 章）",
       table:{cols:["极限状态","承载力"],
         rows:[
           ["块状剪切破坏","Rn = <strong>0.60·Fu·Anv + Ubs·Fu·Ant ≤ 0.60·Fy·Agv + Ubs·Fu·Ant</strong>（φ = 0.75）；Ubs = 1.0 均匀受拉，0.5 不均匀"],
           ["板件剪切屈服","Rn = 0.60·Fy·Agv（φ = 1.00）"],
           ["板件剪切断裂","Rn = 0.60·Fu·Anv（φ = 0.75）"],
           ["板件受拉","屈服 0.90·Fy·Ag / 断裂 0.75·Fu·Ae（节点板用 Whitmore 截面）"]
         ]}},
      {h:"组合构件（SNI 1729:2020 第 I 章）",
       table:{cols:["项目","规定"],
         rows:[
           ["栓钉抗剪","Qn = <strong>0.5·Asa·√(f'c·Ec) ≤ Rg·Rp·Asa·Fu</strong>（I8-1）"],
           ["Rg / Rp 系数","Rg = 1.0（无压型板）/ 0.85 / 0.7；Rp = 0.75（无板）/ 0.6（垂直压型板）"],
           ["组合梁水平剪力 ΣQn","取 0.85·f'c·Ac、Fy·As、ΣQn 中较小者（组合作用程度）"],
           ["包覆/填充组合柱","Pno 计入 钢+混凝土+钢筋；包覆柱含钢率 ≥ 1%"],
           ["混凝土承压","直接承压传递 φ·1.7·f'c·A1（φ = 0.65）"]
         ]}},
      {h:"近似二阶分析 — B1/B2（SNI 1729:2020 附录 8）",
       table:{cols:["项目","表达式"],
         rows:[
           ["放大弯矩 / 内力","Mr = B1·Mnt + B2·Mlt；Pr = Pnt + B2·Plt"],
           ["B1（P-δ，构件）","<strong>B1 = Cm / (1 − α·Pr/Pe1) ≥ 1</strong>，α = 1.0（LRFD）"],
           ["Cm（无横向荷载）","Cm = 0.6 − 0.4·(M1/M2)；支座间有横向荷载时取 1.0"],
           ["Pe1（构件弹性屈曲）","Pe1 = π²·EI* / (Lc1)²，EI* = 0.8·τb·EI（直接分析法）"],
           ["B2（P-Δ，楼层）","<strong>B2 = 1 / (1 − α·Pstory/Pe,story) ≥ 1</strong>，Pe,story = RM·HL/ΔH"]
         ]},
       notes:["B1 放大无侧移（重力）弯矩；B2 放大侧移（水平）效应。直接分析法（C 章）采用折减刚度 0.8·EI 及概念荷载 Ni = 0.002·Yi。"]},
      {h:"构件稳定支撑（SNI 1729:2020 附录 6）",
       table:{center:true,cols:["支撑类型","所需强度","所需刚度"],
         rows:[
           ["柱 — 节间支撑","Vbr = 0.005·Pr","βbr = (1/φ)·(2·Pr/Lbr)，φ = 0.75"],
           ["柱 — 节点支撑","Pbr = 0.01·Pr","βbr = (1/φ)·(8·Pr/Lbr)"],
           ["梁 — 节点（侧向）支撑","Pbr = 0.02·Mr·Cd/ho","βbr = (1/φ)·(10·Mr·Cd)/(Lbr·ho)"]
         ]},
       notes:["Pr / Mr = 被支撑构件所需轴力 / 弯矩。支撑须同时满足强度与刚度；刚度不足将使假定的无支撑长度失效。"]},
      {h:"疲劳设计（SNI 1729:2020 附录 3）",
       table:{cols:["项目","规定"],
         rows:[
           ["适用条件","活载应力循环 &gt; 20,000 次；仅验算使用荷载的净应力幅"],
           ["容许应力幅（A–E′ 类）","<strong>FSR = (6900·Cf/nSR)^0.333 ≥ FTH</strong>（MPa）；Cf = 类别常数，nSR = 循环次数"],
           ["受剪（F 类）","FSR = (690·Cf/nSR)^0.167 ≥ 55 MPa"],
           ["门槛 FTH","低于 FTH 不需验算疲劳（如 A 类 165、C 类 69、E′ 类 18 MPa）"],
           ["细节类别","A（母材）→ E′（最不利焊接细节）；制作与无损检测按 §3.5–3.6"]
         ]}},
      {h:"抗火设计（SNI 1729:2020 附录 4）",
       table:{cols:["项目","规定"],
         rows:[
           ["两种方法","(a) 高温下结构分析；(b) 耐火试验鉴定（耐火等级）"],
           ["钢材性能保留（约）","538°C：E ≈ 0.60·E、Fy ≈ 0.66·Fy；600°C 约 0.31·E、0.47·Fy（表 A-4.2.1）"],
           ["临界温度","构件在火灾荷载组合下达到极限状态的温度"],
           ["火灾荷载组合","1.2D + 0.5L + T（高温作用），按附录 4.1.4"],
           ["对照","SNI 1729 涵盖结构抗火工程；SNI 1741/条例 26 涵盖耐火极限（TKA）— 见消防模块"]
         ]}},
    ...
    sources:"SNI 1729:2020（= AISC 360-16）：B–K 章构件与连接设计、附录 3（疲劳）、附录 4（抗火）、附录 6（稳定支撑）、附录 8（B1/B2 二阶）— 提取自 PDF · AISC 360-22 · AISC 341-22（抗震） · GB 50017-2017（Q 牌号、f = fy/γR、§11 连接、§16 疲劳、§17 抗火）"
  },
  geotechnical:{
    title:"岩土与地基 — 设计参数",
    sections:[
